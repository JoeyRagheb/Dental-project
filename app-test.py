...
def processed_img(img_path):
    ...
    st.info('***Image segmentation in progress***')

    # Specify the source folder for predictions based on the uploaded image
    source_folder = os.path.dirname(img_path) + '/'
    
    # Define the YOLO command with the updated source folder
    yolo_command = f"yolo task=segment mode=predict model=weights/best.pt conf=0.25 source={source_folder} save=true"
